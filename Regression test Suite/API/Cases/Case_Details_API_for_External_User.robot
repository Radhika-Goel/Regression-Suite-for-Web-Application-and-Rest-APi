*** Settings ***
Suite Setup       Common.Suite Setup for Case Details    ${NGCSC9_token}    ${Case ID}
Library           Collections
...
${ssrn}           152115129000
@{Case Fields}    categories    category    closedDate    createDate    customerTrackingNumber    escalation    id
...               lastModifiedDate    owner    priority    reason    status    synopsis    type
...               urgency
...
@{Contact Fields}    addInboundEmailIdToEmailRecipients    ccCustomer    email    followUpMethodPreferrence    name    phone    preferredCallBack
...
Case Detail Problem Description
    ...
    \    ${problem_description_details}    Common.Case Detail API    ${MyJuniper_Segregated_IP}    ${Case Detail Problem Description API}    noteLog    ${NGCSC1_token}
...
Product Fields on Case Details Screen
    ${Case Detail API}    BuiltIn.Catenate    SEPARATOR=/    ${Case Detail API}    ${Case ID}
    BuiltIn.Sleep    60s
    ${product_details}    Common.Case Detail API    ${MyJuniper_Segregated_IP}    ${Case Detail API}    product    ${NGCSC9_token}
...
Account Fields on Case Details Screen
    ${Case Detail API}    BuiltIn.Catenate    SEPARATOR=/    ${Case Detail API}    ${Case ID}
    BuiltIn.Sleep    60s
    ${account_details}    Common.Case Detail API    ${MyJuniper_Segregated_IP}    ${Case Detail API}    account    ${NGCSC9_token}
...
Contact Fields on Case Details Screen
    ${Case Detail API}    BuiltIn.Catenate    SEPARATOR=/    ${Case Detail API}    ${Case ID}
    BuiltIn.Sleep    60s
    ${contact_details}    Common.Case Detail API    ${MyJuniper_Segregated_IP}    ${Case Detail API}    contact    ${NGCSC9_token}
    Log    ${contact_details}
...
Case Detail Problem Description when Case ID doesnt't belong to user
    ...
    Comment    Should Be Equal    ${response.status_code}    ${403}
